...
class Spot:
    ...
    def update_neighbors(self,grid):
        ...
        for i in range(0,4):
            X=self.row+dx[i]
            Y=self.col+dy[i]
            if X>= self.total_rows or Y>= self.total_rows or X<0 or Y<0 or grid[X][Y].is_barrier(): 
                continue
...
def algorithm(draw,grid,start,end):
    ...
    q.put((0,count,start))
    parent = {}
    G_SCORE={spot:float("inf") for row in grid for spot in row}#initilization
    G_SCORE[start]=0
    F_SCORE={spot:float("inf") for row in grid for spot in row}#initilization
